...
class KNN:
    def __init__(self, model_root='knn'):
        recommend_path = os.path.join(model_root, 'recommend_models_%d.pkl' % training_size)
        evaluate_path = os.path.join(model_root, 'evaluate_model_%d.pkl' % training_size)
    ...
    def recommend(self, my_team, their_team, hero_candidates):
        ...
        for candidate, team in team_possibilities:
            ...
            prob_dire = self.recommend_models[candidate-1+heroesTotal].predict_proba(query_dire)[0][0]
